 ...
 *
 * **PTS は受け側の時計 (mp4 の 90kHz)。** 元TSの放送時刻から、焼いた1枚目の
 * 放送時刻を引いてある (`server/live.ts` の `attend`)。fMP4 は必ず 0 から始まり、
 * その 0 がどの放送時刻に当たるかは多重化器が握っていて受け側からは見えないので、
 * **サーバが直してから渡す**。字幕はこの時刻にそのまま置けばよい。
 */
